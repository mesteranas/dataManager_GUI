...
if not os.path.exists(path):
    with open(path,"w",encoding="utf-8") as file:
        file.write('{"contacts":{},"links":{},"passwords":{},"notes":{}}')
class main (qt.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(app.name + _("version : ") + str(app.version))
        layout=qt.QVBoxLayout()
        self.choise=qt.QListWidget()
        self.choise.addItems([_("contacts"),_("links"),_("passwords"),_("notes")])
        self.choise.setAccessibleName(_("select service"))
        layout.addWidget(self.choise)
        self.select=qt.QPushButton(_("select"))
        self.select.setDefault(True)
        self.select.clicked.connect(lambda:gui.CategoryManager(self,self.choise.currentRow()).exec())
